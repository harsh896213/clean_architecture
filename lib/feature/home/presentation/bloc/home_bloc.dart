...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  void _onLoadHomeData(LoadActivities event, Emitter<HomeState> emit) async{
    ...
    emit(HomeDataState(
      ...
      tipOfDay: tipOfDay,
      filterActivity: activitiesList.where((element) => element.timeSlot == (currentState?.selectedChip ?? "Morning"),).toList(),
  ...
  void _filterTimeActivities(FilterTimeActivities event, Emitter<HomeState> emit) async{

    if(state is! HomeDataState) return;

    var currentState = state as HomeDataState ;

    var resultList = currentState.activities.where((element) => element.timeSlot == event.time,).toList();

    emit(currentState.copyWith(filterActivity: resultList, selectedChip: event.time));

  }

  void _nextSelectedTime(
      NextSelectedTime event, Emitter<HomeState> emit) {
  ...
  void _prevSelectedTime(
      PrevSelectedTime event, Emitter<HomeState> emit) {
    if(state is! HomeDataState) return;
